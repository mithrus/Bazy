...
namespace BazyDanych
{
    public partial class Form1 : Form
    {
        ...
        private void BZapytankaPierwsze_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection Polaczenie = new SqlConnection(CiagPolaczenia))
                {
                    string sqlQuery = "Select * from Zlecenie z join ZleceniaNaFakturze zf on z.ZlecenieID=zf.ZlecenieID where GETDATE() < z.TerminZakonczenia and zf.FakturaID in (select f.FakturaID from Faktura f where f.Zaplacona =1)";
                    SqlDataAdapter dataAdapter = new SqlDataAdapter(sqlQuery, Polaczenie);
                    DataSet dataSet = new DataSet();
                    dataAdapter.Fill(dataSet,"wynik");
                    DGVZapytanka.AutoGenerateColumns = true;
                    DGVZapytanka.DataSource = dataSet;
                    DGVZapytanka.DataMember = "wynik";
                }
            }
            catch (Exception ee)
            {
        ...
        private void BZapytankaDrugie_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection Polaczenie = new SqlConnection(CiagPolaczenia))
                {
                    string sqlQuery = "select * from Pracownik p where (select count(*) as Ile from Faktura f where p.PracownikID = f.PracownikID group by f.PracownikID having sum(f.Kwota) > (select avg(kwota) from Faktura)) > 0";
                    SqlDataAdapter dataAdapter = new SqlDataAdapter(sqlQuery, Polaczenie);
                    DataSet dataSet = new DataSet();
                    dataAdapter.Fill(dataSet, "wynik");
                    DGVZapytanka.AutoGenerateColumns = true;
        ...
        private void BZapytankoTrzecie_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection Polaczenie = new SqlConnection(CiagPolaczenia))
                {
                    string sqlQuery = "select top(3) b.nazwa, sum(f.kwota) as kwota from Branza b join ReklamaZBranzy rb on rb.BranzaID=b.BranzaID join Zlecenie z on z.ReklamaID=rb.ReklamaID	join ZleceniaNaFakturze zf on zf.ZlecenieID = z.ZlecenieID	join Faktura f on f.FakturaID = zf.FakturaID group by b.Nazwa order by kwota desc";
                    SqlDataAdapter dataAdapter = new SqlDataAdapter(sqlQuery, Polaczenie);
                    DataSet dataSet = new DataSet();
                    dataAdapter.Fill(dataSet, "wynik");
                    DGVZapytanka.AutoGenerateColumns = true;
        ...
        private void BZapytankoCzwarte_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection Polaczenie = new SqlConnection(CiagPolaczenia))
                {
                    string sqlQuery = "select r.*, COUNT(rl.LokalizacjaID) as ile from Reklama r, ReklamaWLokalizacji rl where rl.ReklamaID = r.ReklamaID group by r.ReklamaID,r.Opis,r.Szerokosc,r.Wysokosc order by ile desc";
                    SqlDataAdapter dataAdapter = new SqlDataAdapter(sqlQuery, Polaczenie);
                    DataSet dataSet = new DataSet();
                    dataAdapter.Fill(dataSet, "wynik");
                    DGVZapytanka.AutoGenerateColumns = true;
